...
- [No.60 n个骰子的点数](#n个骰子的点数)//比较难的动态规划和压缩数组的优化算法、类似背包问题
...
//优化空间后的一维数组
//一定要注意：比较关键的两步，一步是“dp[j] = 0;”，一步是“if (j - cur < i-1)”。
//对于前一步，因为是从后往前逐个累加，在加到当前点数时，必须把原先存放的i-1个骰子的数据置0，否则累加出错。对于后一步，如果不加此判据，会取到i-2个骰子的数据，此时可认为是“脏数据”，会导致累加出错。从实际情况来分析，i-1个骰子的最小值就是i-1，不会比这再小，因此此处的判据是 i-1，而不是0；
class Solution {
public:
    vector<double> twoSum(int n) {
        int dp[70];
