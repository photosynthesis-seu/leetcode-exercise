...
+ 因此，向内收窄短板可以获取面积最大值。换个角度理解：
  ...
  在状态S(i,j)下向内移动短板至S(i+1,j)（假设 h[i] < h[j]h[i]<h[j] ），则相当于消去了 {S(i, j - 1), S(i, j - 2), ... , S(i, i + 1)}S(i,j−1),S(i,j−2),...,S(i,i+1) 状态集合。而所有消去状态的面积一定<=S(i,j)：
  
  短板高度：相比S(i,j) 相同或更短（<= h[i]<=h[i]）；底边宽度：相比S(i,j) 更短。因此所有消去的状态的面积都S(i,j)。通俗的讲，我们每次向内移动短板，所有的消去状态都不会导致丢失面积最大值 。
