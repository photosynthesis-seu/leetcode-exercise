...
+ 动态规划
  ...
  - [面试题17.13 恢复空格](https://github.com/photosynthesis-seu/leetcode-exercise/blob/master/%E9%99%84%E5%BD%952.md#恢复空格)//重点关注！动态规划+字典树！
+ DFS、BFS算法（回溯算法、分治算法->递归实现）
  ...
  - [No.679 二十四点游戏](https://github.com/photosynthesis-seu/leetcode-exercise/blob/master/%E9%99%84%E5%BD%952.md#24点游戏)//DFS深度遍历每一种情况
...
面试题01.06 字符串压缩(遍历算法、字符串相加、to_string函数实现整数转字符串、自己实现整数转字符串)
面试题01.07 旋转矩阵 
面试题02.01 移除重复节点(使用哈希表去重)
面试题08.11 硬币（核心就是动态规划里的背包问题！）
面试题16.11 跳水板(数学方法、set暴力解法)
面试题16.03 交点（主要考察你细不细致）
面试题17.13 恢复空格（重点关注！动态规划+字典树！）
...
+ **一定要区分i++和++i**
  - i++ 与 ++i 的主要区别：
    ...
    ```
    - i++ 不能作为左值，而++i 可以。
  - 在for循环中，i++是先引用i以后再加1，++i是加1后再引用i，在不引用i的值的表达式的情况下，两者的效果是一样的。
    - 例如：for(int i=1;i<len;i++)和for(int i=1;i<len;++i)一样。但是for(int i=1;i<len;int n=i++)情况就不同了。
    - 但是i++要另外申请一个变量来保存i加一之后的值，因为i和i加一的值都要用到。而++i不用另外申请一个变量来保存i加一之后的值。因此++i的语句在汇编代码中比i++更优化，效率也更高。
